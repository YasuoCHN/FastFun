...
public class AgentPayQueryHelper {
	
	private static Logger logger = LoggerFactory.getLogger(AgentPayQueryHelper.class);
	
	
	public static String queryAgentPay(OrderReqInfo info) {
	...
	private static String getOnlineSMS163(OrderReqInfo info) {
		...
		try  {
			...
			String url = "http://47.75.32.235/api/v1.0/agentPay/query";
			...
			logger.info("南京众誉sdD7查询代付结果:"+reqResult);
			...
			if (reqResult != null && reqResult.length() > 0) {
				...
				String angentStatus = jsonobj.getString("paySt");
				...
				if (checksign) {
					...
					resmap.put("angentStatus", angentStatus);
				} else {
					resmap.clear();
					resmap.put("status", "-2");
			...
			logger.info("南京众誉sdD7查询代付生成json:"+result);
		} catch (Exception e) {
			logger.error("南京众誉sdD7查询代付失败!");
